...
contract OPPStaking is Ownable {
    ...
    address public constant OPTIMISM_PRIME_TOKEN =
    ...
    function InitializeStaking(uint256 initialDeposit) external onlyOwner {
        ...
        IERC20(OPTIMISM_PRIME_TOKEN).safeTransferFrom(
            msg.sender,
            address(this),
            rewards
        );
    ...
    function claimRewards() public {
        if (block.timestamp <= END_STAKING_UNIX_TIME) {
            ...
            if (userRewards > 0) {
                IERC20(OPTIMISM_PRIME_TOKEN).safeTransfer(
                    msg.sender,
                    userRewards
                );
            }
    ...
    function getNewAccumulator() internal view returns (uint256) {
        if (totalStaked == 0) {
            ...
            if (block.timestamp <= END_STAKING_UNIX_TIME) {
                uint256 numerator = OPP_PER_SECOND *
                ...
                uint256 numerator = OPP_PER_SECOND *
                    (END_STAKING_UNIX_TIME - lastUpdate) *
                    1e24;
                uint256 denominator = totalStaked;
